...
import org.atlanmod.ocl.{OclAny, OclType, Bag, Set, Sequence, Collection}

class BasicInteger(var value: Int) extends OclAny with OclType with Serializable {

  final val OCL_TYPE_NAME = "BasicInteger";

  override def equals(that: Any): Boolean =
    that match {
      case that: BasicInteger => {
        that.canEqual(this) &&
  ...
  def canEqual(a: Any) = a.isInstanceOf[BasicInteger]
  ...
  override def +(other: OclAny): OclAny = {
    other match {
      case other: BasicInteger => {
        return new BasicInteger(value + other.value)
      }
      case _ => {
  ...
  override def refGetValue(s: String): OclAny = {
    s match {
      case "value" => new BasicInteger(value)
